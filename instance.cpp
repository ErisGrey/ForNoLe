...
{
    instanceName = GetFileName(instanceFile);
   
    
    paramfileName = GetFileName(paramsFile);
    ...
    dconfig = new DroneConfig(1,
        ...
        voltage,
        battery_reserve);
    dconfig->flightTime.resize(num_nodes, pair<long, long> (-1, -1));
    ...
    for (int customer = 1; customer < getNum_nodes(); ++customer) {
        ...
        if (optimal_setting[0] != -1) {
            ...
            dconfig->flightTime[customer].second =
                ...
                energyModel->h_flight / energyModel->v_landing  //  time landing in second
                ;
            dconfig->flightTime[customer].first = round(dconfig->flightTime[customer].first / 60); // /60 to minute
            dconfig->flightTime[customer].second = round(dconfig->flightTime[customer].second / 60); // /60 to minute
...
void Instance::read_input(const string& inputFile)
{
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        weight[i] = round(weight[i] * 100)/100;
    }
    myFile.close();
}
